...
export const USER_NOTE_VIEW_REQUEST   = 'ASYNC_USER_NOTE_VIEW_REQUEST';
export const USER_NOTE_VIEW_SUCCESS   = 'ASYNC_USER_NOTE_VIEW_SUCCESS';
export const USER_NOTE_VIEW_FAILURE   = 'ASYNC_USER_NOTE_VIEW_FAILURE';
...
export function viewRemoteNote(id) {
    return async dispatch => {
        dispatch(request());
        try {
            const result = await UserService.viewRemoteNote(id);
            dispatch(success(result));
            return result;
        } catch (error) {
            dispatch(failure(error.toString()));
    ...
    function request() { return { type: USER_NOTE_VIEW_REQUEST } }
    function success(data) { return { type: USER_NOTE_VIEW_SUCCESS, data } }
    function failure(error) { return { type: USER_NOTE_VIEW_FAILURE, error } }
}